...
func TestStream_PluginHooksFire(t *testing.T) {
	...
	plugin := bond.NewHooksPlugin("spy", func(r *bond.HookRegistry) {
		bond.On[*bond.BeforeStreamHook](r, func(ctx context.Context, e *bond.BeforeStreamHook) error {
			beforeStream = true
			return nil
		})
		bond.On[*bond.AfterStreamHook](r, func(ctx context.Context, e *bond.AfterStreamHook) error {
			afterStream = true
			return nil
		})
	})
...
func TestStream_BeforeToolCallHookAbort(t *testing.T) {
	plugin := bond.NewHooksPlugin("blocker", func(r *bond.HookRegistry) {
		bond.On[*bond.BeforeToolCallHook](r, func(ctx context.Context, e *bond.BeforeToolCallHook) error {
			return bond.ErrAbort
		})
	})
