...
func TestProperty_JSONRPCMessageRoundTrip(t *testing.T) {
	f := func(msg Message) bool {
		...
		go func() {
			errCh <- transport.WriteMessage(msg)
			pw.Close()
		}()
...
// Generate implements quick.Generator for Message, producing valid JSON-RPC
...
func generateError(rand *rand.Rand) *ErrorObject {
	codes := []int{CodeParseError, CodeInvalidRequest, CodeMethodNotFound, CodeInvalidParams, CodeServerNotInit, CodeNoActiveSession}
	errObj := &ErrorObject{
